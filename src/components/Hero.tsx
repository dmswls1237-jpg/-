        ...
        >
          ...
          <div className="flex flex-col justify-center lg:col-span-7">
            ...
            <motion.div variants={itemVariants} className="mb-4 inline-flex items-center space-x-2">
              <span className="rounded bg-brand-gold/15 border border-brand-gold/30 px-3.5 py-1.5 text-sm font-bold tracking-wide text-brand-gold">
                #부산 지역 사건 집중 #신속한 상담 진행
              </span>
            </motion.div>

            {/* Slogan & Title */}
            <motion.h1 variants={itemVariants} className="font-serif text-4xl font-extrabold leading-tight tracking-tight text-white sm:text-5xl lg:text-5xl">
              법률 문제,<br />
            ...
            <motion.p variants={itemVariants} className="mt-6 text-base leading-relaxed text-slate-250 md:text-lg">
            ...
            <motion.div variants={itemVariants} className="mt-8 flex flex-col space-y-3 sm:flex-row sm:space-x-4 sm:space-y-0">
              <button
                ...
                className="flex items-center justify-center space-x-2 rounded-sm bg-brand-gold px-8 py-4.5 text-center text-base font-bold text-brand-navy shadow-lg transition duration-200 hover:bg-brand-gold-light cursor-pointer active:scale-95"
              >
                <Calendar className="mr-1 shadow-xs h-5 w-5" />
                <span>방문/전화 상담 예약하기</span>
              </button>
              
              <a
                href="tel:051-711-4509" // Interactive direct dial for users
                className="flex items-center justify-center space-x-2 rounded-sm border border-slate-700 bg-slate-900/60 px-6 py-4 text-center text-base font-bold text-slate-200 transition duration-200 hover:bg-slate-900 hover:text-white hover:border-brand-gold"
              >
                <Phone className="mr-1 h-4 w-4 text-brand-gold animate-bounce" />
            ...
            >
              {[
                ...
                <div key={idx} className="border-l border-brand-gold/30 pl-4">
                  <p className="text-sm sm:text-base font-bold text-slate-100">{badge.label}</p>
                  <p className="text-xs sm:text-sm text-slate-400 mt-1 leading-normal">{badge.desc}</p>
                </div>
              ))}
            </motion.div>
          </div>
          ...
          <div className="flex items-center justify-center lg:col-span-5">
            <motion.div 
              variants={itemVariants}
              className="relative w-full max-w-sm rounded-sm border border-slate-800 bg-slate-950/80 p-8 shadow-2xl backdrop-blur-md"
            >
              {/* Highlight gold framing bar */}
              <div className="absolute top-0 left-0 h-1 w-full bg-brand-gold scale-x-50 origin-left" />
              
              <Landmark className="mb-6 h-8 w-8 text-brand-gold" />
              
              <h3 className="font-serif text-lg font-bold text-brand-gold">
                “결과를 넘어, 의뢰인의 일상 회복까지.”
              </h3>
              
              <p className="mt-4 text-xs sm:text-sm leading-relaxed text-slate-350">
              ...
              <div className="mt-8 rounded bg-slate-900/80 p-4 border border-slate-800">
